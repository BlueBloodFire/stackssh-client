    ...
    <div className="flex gap-3 py-1.5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-[12px] font-semibold tracking-wide" style={{ color: dotColor }}>{label}</span>
          {step.stepIndex && (
            <span className="text-[11px]" style={{ color: colors.textDim }}>#{step.stepIndex}</span>
          )}
        </div>
        {step.toolName && (
          <div className="text-[12px] px-2.5 py-1.5 rounded-xl mb-1" style={{
        ...
        {step.toolResult && (
          <pre className="text-[12px] px-2.5 py-2 rounded-xl mb-1 overflow-x-auto" style={{
            backgroundColor: colors.bgSecondary,
            border: `1px solid ${colors.border}`,
            color: colors.text,
            fontFamily: 'monospace',
        ...
        {step.content && step.stepType !== 'result' && (
          <div className="text-[13px] leading-6" style={{ color: colors.textSecondary, whiteSpace: 'pre-wrap' }}>
            {step.content.length > 300 ? step.content.slice(0, 300) + '...' : step.content}
          </div>
        )}
        {step.error && (
          <div className="text-[12px] px-2.5 py-2 rounded-xl" style={{
    ...
    return (
      <div className="px-4 py-1.5 flex justify-end">
        <div className="flex flex-col items-end max-w-[88%]">
          <div
            className="px-4 py-2.5 text-[14px] leading-7 rounded-2xl"
            style={{
              backgroundColor: `${colors.accent}14`,
              color: colors.text,
              border: `1px solid ${colors.accent}24`,
              whiteSpace: 'pre-wrap',
...
export function RightSidebar({ width = 400, activeTerminalSessionId }: RightSidebarProps) {
  ...
  const boundConnection = activeBinding
    ? connections.find((c) => c.id === activeBinding.connectionId)
    : connections.find((c) => c.id === currentConnectionId)
  const activeEditorTab = activeTabKey ? openTabs.find((tab) => tab.key === activeTabKey) : null
  const activeFolderPath = activeConnectionId ? currentPathByConnection[activeConnectionId] || '/' : null
  const sessionHeadline = currentSession?.messages.find((message) => message.role === 'user')?.content?.slice(0, 48) || '新会话'
  const currentModelLabel = modelForm.model || '未配置模型'
  const currentContextChips = [
    activeEditorTab ? `文件 · ${activeEditorTab.name}` : null,
    activeFolderPath ? `目录 · ${activeFolderPath}` : null,
    activeBinding ? `终端 · ${activeBinding.connectionName}` : null,
    inputTags.length > 0 ? `上下文标签 · ${inputTags.length}` : null,
  ...
  return (
    <div className="flex flex-col h-full flex-shrink-0 overflow-hidden" style={{ width, backgroundColor: colors.bgPrimary }}>
      {(() => {
        ...
        return (
          <div className="flex items-center gap-2 px-4 py-2 border-b" style={{ backgroundColor: connected ? `${colors.accent}08` : `${colors.textDim}06`, borderColor: colors.border }}>
            <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: connected ? '#22c55e' : colors.textDim }} />
            <span className="text-[12px] truncate" style={{ color: colors.textDim }}>
      ...
      <div className="px-4 pt-3 pb-3 border-b space-y-3" style={{ backgroundColor: colors.bgSecondary, borderColor: colors.border }}>
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-xl px-3 py-2.5" style={{ backgroundColor: colors.bgPrimary, border: `1px solid ${colors.border}` }}>
            <div className="text-[11px] font-semibold mb-1" style={{ color: colors.textDim }}>当前会话</div>
            <div className="text-[13px] leading-5" style={{ color: colors.text }}>{sessionHeadline}</div>
          </div>
          <div className="rounded-xl px-3 py-2.5" style={{ backgroundColor: colors.bgPrimary, border: `1px solid ${colors.border}` }}>
            <div className="text-[11px] font-semibold mb-1" style={{ color: colors.textDim }}>执行模型</div>
            <div className="text-[13px] leading-5 truncate" style={{ color: colors.text }}>{currentModelLabel}</div>
          </div>
          <div className="rounded-xl px-3 py-2.5" style={{ backgroundColor: colors.bgPrimary, border: `1px solid ${colors.border}` }}>
            <div className="text-[11px] font-semibold mb-1" style={{ color: colors.textDim }}>绑定连接</div>
            <div className="text-[13px] leading-5 truncate" style={{ color: colors.text }}>{boundConnection ? boundConnection.name : '未绑定'}</div>
          </div>
          <div className="rounded-xl px-3 py-2.5" style={{ backgroundColor: colors.bgPrimary, border: `1px solid ${colors.border}` }}>
            <div className="text-[11px] font-semibold mb-1" style={{ color: colors.textDim }}>Agent 能力组合</div>
            <div className="text-[13px] leading-5 truncate" style={{ color: colors.text }}>{currentAgent?.agentName || '未选择 Agent'}</div>
          </div>
        </div>

        <div className="flex flex-wrap gap-2">
          <button onClick={() => currentAgentId && newConversation(currentAgentId)} className="px-2.5 py-1.5 rounded-xl text-[12px] font-medium transition-all" style={{ backgroundColor: colors.accentSoft, color: colors.accent, border: `1px solid ${colors.accent}22` }}>
            新建会话
          </button>
          <button onClick={() => setShowModelConfig(!showModelConfig)} className="px-2.5 py-1.5 rounded-xl text-[12px] font-medium transition-all" style={{ backgroundColor: colors.bgTertiary, color: showModelConfig ? colors.accent : colors.textSecondary, border: `1px solid ${showModelConfig ? colors.accent : colors.border}` }}>
            模型配置
          </button>
          <button className="px-2.5 py-1.5 rounded-xl text-[12px] font-medium transition-all" style={{ backgroundColor: colors.bgTertiary, color: colors.textSecondary, border: `1px solid ${colors.border}` }}>
            历史记录
          </button>
          <button className="px-2.5 py-1.5 rounded-xl text-[12px] font-medium transition-all" style={{ backgroundColor: colors.bgTertiary, color: colors.textSecondary, border: `1px solid ${colors.border}` }}>
            上下文管理
        ...
        {currentContextChips.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {currentContextChips.map((chip) => (
              <span
                key={chip}
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {!currentSession ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 px-6">
            ...
            <h3 className="text-[18px] font-medium" style={{ color: colors.text }}>开始任务</h3>
            <p className="text-[13px] text-center max-w-xs leading-6" style={{ color: colors.textSecondary }}>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4 px-6">
            ...
            <div className="text-center">
              <p className="text-[15px] font-medium mb-1" style={{ color: colors.text }}>StackSSH AI</p>
              <p className="text-[13px]" style={{ color: colors.textDim }}>执行命令 · 排查问题 · 管理服务器</p>
            </div>
            <div className="w-full max-w-xs space-y-1.5">
              {['检查服务器状态', '分析日志文件', '部署应用', '排查报错'].map((text) => (
                ...
                }} className="w-full text-left px-3.5 py-2.5 rounded-xl text-[13px] transition-colors hover:opacity-80" style={{ color: colors.textSecondary, backgroundColor: colors.bgTertiary, border: `1px solid ${colors.border}` }}>
                  {text}
                </button>
              ))}
            </div>
          ...
          <div className="py-3">
            ...
            {isLoading && (
              <div className="px-4 py-2 flex justify-start">
                <div className="px-4 py-3 flex items-center gap-2 rounded-2xl" style={{ backgroundColor: colors.bgTertiary, border: `1px solid ${colors.border}` }}>
                  ...
                  <span className="text-[12px]" style={{ color: colors.textDim }}>思考中...</span>
                </div>
              </div>
            )}
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t flex-shrink-0" style={{ backgroundColor: colors.bgSecondary, borderColor: colors.border }}>
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[12px] font-medium transition-all" style={{ backgroundColor: colors.bgTertiary, color: colors.textSecondary, border: '1px solid transparent' }}>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => clearInputTags()} className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-[12px] font-medium transition-all" style={{ backgroundColor: colors.bgTertiary, color: colors.textSecondary, border: '1px solid transparent' }} title="清空上下文">
            清空上下文
          </button>
          <button onClick={() => currentAgentId && newConversation(currentAgentId)} className="flex items-center gap-1 px-2 py-1.5 rounded-xl text-[12px] font-medium transition-all" style={{ backgroundColor: colors.bgTertiary, color: colors.textSecondary, border: '1px solid transparent' }} title="新建会话">
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <line x1="12" y1="5" x2="12" y2="19"></line>
              <line x1="5" y1="12" x2="19" y2="12"></line>
            </svg>
          </button>
          <button className="flex items-center gap-1 px-2 py-1.5 rounded-xl text-[12px] font-medium transition-all" style={{ backgroundColor: colors.bgTertiary, color: colors.textSecondary, border: '1px solid transparent' }} title="历史记录">
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className="flex flex-col relative px-4 pt-2 pb-3 flex-shrink-0" style={{ backgroundColor: colors.bgSecondary }}>
        ...
        {showModelConfig && (
          <div className="mb-2 rounded-lg border p-3 space-y-2.5" style={{ backgroundColor: colors.bgPrimary, borderColor: colors.border }}>
            ...
            <div className="flex items-center justify-between">
              <span className="text-[12px] font-semibold" style={{ color: colors.textSecondary }}>模型配置</span>
            ...
            {modelLoading ? (
              <div className="flex items-center gap-2 py-2">
                ...
                <span className="text-[12px]" style={{ color: colors.textDim }}>加载中...</span>
              </div>
            ) : (
              <>
                {/* ── 内置预设快选 ── */}
                <div>
                  <p className="text-[11px] mb-1.5" style={{ color: colors.textDim }}>快速切换</p>
                ...
                {modelProfiles.length > 0 && (
                  <div>
                    <p className="text-[11px] mb-1.5" style={{ color: colors.textDim }}>我的配置</p>
                    <div className="flex flex-wrap gap-1">
                      {modelProfiles.map(p => {
                        ...
                        return (
                          <div key={p.id} className="group flex items-center gap-0.5">
                            <button
                              ...
                              disabled={modelSaving}
                              className="px-2.5 py-1 rounded-l text-[11px] font-medium transition-colors"
                              style={{
                                backgroundColor: active ? colors.accent : colors.bgTertiary,
                                color: active ? '#fff' : colors.textSecondary,
                                border: `1px solid ${active ? colors.accent : colors.border}`,
                            ...
                            <button
                              ...
                              className="px-1.5 py-1 rounded-r text-[10px] opacity-0 group-hover:opacity-100 transition-opacity"
                              style={{
                                backgroundColor: active ? colors.accent : colors.bgTertiary,
                ...
                <div className="space-y-1.5 pt-0.5" style={{ borderTop: `1px solid ${colors.border}` }}>
                  <div className="pt-1.5">
                    <label className="block text-[11px] mb-1" style={{ color: colors.textDim }}>模型</label>
                    <input
                      value={modelForm.model}
                      onChange={e => setModelForm(f => ({ ...f, model: e.target.value }))}
                      placeholder="deepseek-chat / gpt-4o / qwen-plus"
                      className="w-full text-[12px] px-2.5 py-2 rounded outline-none"
                      style={{ backgroundColor: colors.bgInput, color: colors.text, border: `1px solid ${colors.border}` }}
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] mb-1" style={{ color: colors.textDim }}>API Key</label>
                    <div className="flex gap-1">
                      <input
                        ...
                        placeholder="sk-..."
                        className="flex-1 text-[12px] px-2.5 py-2 rounded outline-none"
                        style={{ backgroundColor: colors.bgInput, color: colors.text, border: `1px solid ${colors.border}` }}
                      />
                  ...
                  <div>
                    <label className="block text-[11px] mb-1" style={{ color: colors.textDim }}>Base URL</label>
                    <input
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={modelSaving}
                    className="flex-1 py-2 rounded text-[12px] font-medium transition-colors"
                ...
                {showSaveProfile && (
                  <div className="space-y-1.5 p-2 rounded" style={{ backgroundColor: colors.bgSecondary, border: `1px solid ${colors.border}` }}>
                    <input
                      ...
                      autoFocus
                    />
                    <label className="flex items-center gap-1.5 text-[11px] cursor-pointer" style={{ color: colors.textDim }}>
                      <input
                        type="checkbox"
        ...
        <div className="relative w-full rounded-lg border transition-all flex flex-col" style={{ backgroundColor: colors.bgInput, borderColor: isFocused ? `${colors.accent}80` : colors.border, boxShadow: isFocused ? `0 0 0 1px ${colors.accent}30` : 'none' }}>
          {inputTags.length > 0 && (
            <div className="flex flex-wrap gap-2 px-3 pt-3 pb-1 max-h-[100px] overflow-y-auto">
              {inputTags.map((tag) => (
                <div key={tag.id} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[12px] max-w-[220px]" style={{ backgroundColor: colors.bgTertiary, border: `1px solid ${colors.border}`, color: colors.textSecondary }}>
          ...
          <div
            ...
            className="w-full bg-transparent resize-none outline-none text-[14px] leading-7 flex-1 whitespace-pre-wrap break-words min-h-[132px] max-h-[300px] overflow-y-auto"
            style={{
          ...
          {(!inputText || inputText.trim() === '') && (
            <div className="absolute pointer-events-none text-[14px]" style={{ left: '16px', top: inputTags.length > 0 ? '46px' : '10px', color: colors.textDim, opacity: 0.6 }}>
              {inputPlaceholder()}
            </div>
          )}
            ...
            <div className="relative">
              ...
              {showAttachmentMenu && (
                <div className="absolute bottom-full right-0 mb-1 w-32 rounded-lg border shadow-lg py-1 z-50" style={{ backgroundColor: colors.bgPrimary, borderColor: colors.border }}>
                  <button onClick={handleAddCurrentFile} disabled={!activeTabKey} className="w-full text-left px-3 py-2 text-[12px] hover:bg-black/5 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" style={{ color: colors.text }}>
                    添加当前文件
                  </button>
                  <button onClick={handleAddCurrentFolder} disabled={!activeConnectionId} className="w-full text-left px-3 py-2 text-[12px] hover:bg-black/5 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" style={{ color: colors.text }}>
                    添加当前目录
                  </button>
                  <button onClick={handleAddSelectedText} className="w-full text-left px-3 py-2 text-[12px] hover:bg-black/5 transition-colors" style={{ color: colors.text }}>
                    添加选中文本
                  </button>
                </div>
              )}
        ...
        <div className="flex items-center mt-2 text-[12px]" style={{ color: colors.textDim }}>
          <div className="flex items-center gap-1.5">
            <div className="relative" style={{ zIndex: 10 }}>
              <select value={currentAgentId || ''} onChange={(e) => setCurrentAgentId(e.target.value)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md cursor-pointer transition-colors appearance-none pr-6" style={{ backgroundColor: colors.bgTertiary, color: colors.textSecondary, fontSize: '12px', border: 'none' }}>
          ...
          <div className="relative">
            <button onClick={() => setShowSendModeDropdown(!showSendModeDropdown)} className="flex items-center gap-1 px-2 py-1 rounded-md cursor-pointer transition-colors hover:bg-black/10" style={{ backgroundColor: 'transparent', color: colors.textDim }} title="点击选择发送快捷键">
              ...
              {sendOnEnter ? (
                <span style={{ fontSize: '12px', fontFamily: 'monospace' }}>Enter 发送</span>
              ) : (
                <span style={{ fontSize: '12px', fontFamily: 'monospace' }}>{currentKeyLabel} 发送</span>
              )}
            </button>
            {showSendModeDropdown && (
              <div className="absolute bottom-full right-0 mb-1 rounded-lg border shadow-lg py-1 min-w-[140px]" style={{ backgroundColor: colors.bgPrimary, borderColor: colors.border }}>
                <button onClick={() => selectSendMode('enter')} className="w-full flex items-center gap-2 px-3 py-1.5 text-left transition-colors" style={{ fontSize: '12px', color: sendOnEnter ? colors.accent : colors.textSecondary }}>
                  {sendOnEnter && (
                    <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <polyline points="20 6 9 17 4 12"></polyline>
                    </svg>
                ...
                <button onClick={() => selectSendMode('cmd')} className="w-full flex items-center gap-2 px-3 py-1.5 text-left transition-colors" style={{ fontSize: '12px', color: !sendOnEnter ? colors.accent : colors.textSecondary }}>
                  {!sendOnEnter && (
                    <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <polyline points="20 6 9 17 4 12"></polyline>
                    </svg>
